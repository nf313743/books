...
let findTransactionsOnDisk owner: Guid * seq<Transaction> =
    let accountDir = findAccountFolder owner
    match accountDir with
    | "" -> (Guid.NewGuid(), Seq.empty)
    | _ ->
        let accountId =
            accountDir.Substring(accountDir.IndexOf("_") + 1)
            |> Guid.Parse
...
let writeTransaction (accountId: Guid) (owner: string) (transaction: Transaction) =
    ...
    File.WriteAllText(filePath, Transactions.serialized transaction)
